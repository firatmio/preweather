...
import { FaCloudRain, FaExclamation, FaLocationDot, FaSnowflake, FaTemperatureHalf, FaWater, FaWind } from 'react-icons/fa6'
...
interface GeoSuggestion { display: string; lat: number; lon: number }
...
export default function APP() {
  ...
  const previousViewRef = useRef<{ center: L.LatLngExpression, zoom: number }>({ center: [39.0, 33.0], zoom: 6 })
  ...
  useEffect(() => {
    try { localStorage.setItem('pw.tempUnit', tempUnit) } catch {}
  }, [tempUnit])
  useEffect(() => {
    try { localStorage.setItem('pw.windUnit', windUnit) } catch {}
  }, [windUnit])

  // İlk render flash engelle: body'ye preload class, hydrate sonrası kaldır
  useEffect(() => {
    document.body.classList.add('preload')
    const h = setTimeout(()=> { document.body.classList.remove('preload') }, 60)
    return () => { clearTimeout(h); document.body.classList.remove('preload') }
  }, [])
    ...
    try {
      ...
      metricNames.forEach(m => {
        ...
        if (obj && typeof obj === 'object') {
          Object.keys(obj).forEach(k => { if (k !== 'percentage') dateKeySet.add(k) })
        }
      })
      ...
      metricNames.forEach(m => {
        ...
        const row = [m, String(pct), ...dateKeys.map(k => obj && k in obj ? String(obj[k]) : '')]
        rows.push(row)
      })
      if (typeof prediction.percentage !== 'undefined') {
        rows.unshift(['OVERALL', String(prediction.percentage), ...dateKeys.map(()=> '')])
      }
  ...
  function ClickCapture() {
    useMapEvents({
      click(ev) {
        ...
        const changed = !point || Math.abs(point.lat - newPoint.lat) > 0.00005 || Math.abs(point.lng - newPoint.lng) > 0.00005
        if (mapRef.current) {
          previousViewRef.current = { center: mapRef.current.getCenter(), zoom: mapRef.current.getZoom() }
        }
        setPoint(newPoint)
        if (changed) {
          setResult(null)
  ...
  const metricsList = [
    'ALLSKY_KT','AOD_55_ADJ','CDD18_3','FROST_DAYS','HDD18_3','PRECSNOLAND','PRECTOTCORR','RH2M','SNODP','T2M','T2M_MAX','T2M_MIN','WS2M'
  ]
  ...
  const handleFetchPrediction = async () => {
    ...
    try {
      ...
      const res = await fetch('https://hackathon.hegsam.casa/calculate', {
        method: 'POST',
        headers: {
      ...
      if (!res.ok) {
        ...
        if (res.status === 500 && lowered.includes('could not parse response from prophet')) {
  ...
  const parsedAdvice = useMemo(() => {
    if (!result?.agricultural_advice) return null
    const raw: string = result.agricultural_advice
  ...
  const orderedAdviceEntries = useMemo(() => {
    ...
    const order = ['yorum','öneri','comment','advice','summary','risks','recommendations','actions','notes']
  ...
  // Düz metin dönen agricultural_advice için satır/parça işleme
  const processedAdviceLines = useMemo(() => {
    if (parsedAdvice) return []
    const raw = result?.agricultural_advice
    if (!raw || typeof raw !== 'string') return []
    // Satırlara böl, boşları ele
    const lines = raw.split(/\r?\n+/).map(l=>l.trim()).filter(Boolean)
    const out: { labelKey: string | null; content: string }[] = []
    ...
    lines.forEach(line => {
      const m = line.match(/^([A-Za-zÇÖÜĞİŞçöüğışÂâİıÖöŞşÜü]+)\s*:\s*(.*)$/)
      if (m) {
        const keyRaw = m[1].toLowerCase()
        const rest = m[2].trim()
        const labelKey = labelMap[keyRaw] || null
        out.push({ labelKey, content: rest })
      } else {
        // Önceki satır label ise devam satırı olabilir -> ekle
        if (out.length && !out[out.length-1].content.endsWith('\n')) {
          out[out.length-1].content += '\n' + line
        } else {
          out.push({ labelKey: null, content: line })
        }
      }
    })
    return out
  }, [parsedAdvice, result?.agricultural_advice])
  ...
  const reverseGeocode = async (pt: SelectedPoint) => {
    try {
      ...
      setWaterDetected(false)
      waterDetectedRef.current = false
      const url = `https://nominatim.openstreetmap.org/reverse?lat=${pt.lat}&lon=${pt.lng}&format=jsonv2&accept-language=${language}`
      ...
      const district = addr.suburb || addr.county || addr.village || addr.municipality || ''
      const parts = [country, city, district].filter(Boolean)
      const label = parts.length ? parts.join(', ') : (data.display_name || `${pt.lat},${pt.lng}`)
      setAddress(label)
      setAddrQuery(label)
      const waterFlag = addr.ocean || addr.sea || addr.water || addr.bay || addr.hafen
      if (!country || waterFlag) { triggerAddrError(); setAddrInvalid(true); setWaterDetected(!!waterFlag); waterDetectedRef.current = !!waterFlag }
      // Ek su tespiti: Overpass ile yakındaki water polygon kontrolü (cache'li)
      if (!waterFlag) {
        checkWaterViaOverpass(pt).then(isWater => {
          if (isWater) { triggerAddrError(); setAddrInvalid(true); setWaterDetected(true); waterDetectedRef.current = true }
        }).catch(()=>{})
      }
    } catch (e) {
      setAddress(t('addr.reverseError'))
      triggerAddrError(); setAddrInvalid(true); setWaterDetected(false); waterDetectedRef.current = false
    } finally {
      setAddrLoading(false)
    }
  }

  useEffect(()=> { if(point) reverseGeocode(point) }, [point, language])

  // Forward geocode suggestions
  useEffect(()=> {
    if (!editingAddress) return
    const q = addrQuery.trim()
    if (!q) { setGeoSuggestions([]); return }
    setAddrLoading(true)
    const searchId = ++lastSearchIdRef.current
    const controller = new AbortController()
    const handler = setTimeout(async () => {
      try {
        const url = `https://nominatim.openstreetmap.org/search?format=jsonv2&addressdetails=1&limit=6&accept-language=${language}&q=${encodeURIComponent(q)}`
        const res = await fetch(url, { signal: controller.signal, headers: { 'Accept': 'application/json' } })
        if (!res.ok) throw new Error('bad response')
        const data = await res.json()
        if (lastSearchIdRef.current !== searchId) return // stale
        const mapped: GeoSuggestion[] = data.map((d:any) => ({ display: d.display_name, lat: parseFloat(d.lat), lon: parseFloat(d.lon) }))
  ...
  const checkWaterViaOverpass = async (pt: SelectedPoint): Promise<boolean> => {
    ...
    if (waterCacheRef.current.has(key)) return waterCacheRef.current.get(key) || false
    try {
      ...
      const res = await fetch('https://overpass-api.de/api/interpreter', { method:'POST', body: new URLSearchParams({ data: query }), signal: controller.signal })
      ...
      if (Array.isArray(data.elements)) {
        ...
        for (const el of data.elements) {
          ...
          if (pts.length < 3) continue
            ...
            if (approxM2 > 5000) { isWater = true; break }
        }
      }
  ...
  const renderHighlighted = (text: string) => {
    ...
    try {
      ...
      while ((match = re.exec(text)) !== null) {
        ...
        parts.push(<span className="addr-sugg-highlight" key={match.index}>{match[0]}</span>)
  ...
  const handleSuggestionNav = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === 'ArrowDown') {
      ...
      e.preventDefault()
      setSuggIndex(i => (i - 1 + geoSuggestions.length) % geoSuggestions.length)
    } else if (e.key === 'Enter') {
  ...
  useEffect(()=> { if (!editingAddress) setSuggIndex(-1) }, [editingAddress])
  useEffect(()=> { if (!geoSuggestions.length) setSuggIndex(-1) }, [geoSuggestions])
  ...
  useEffect(()=> {
    ...
    if (eOffsetTop < cTop) {
      ...
      container.scrollTo({ top: eOffsetTop - cHeight + eHeight + 4, behavior: 'smooth' })
    }
  }, [suggIndex])
  ...
  useEffect(()=> {
    ...
    if (pendingAutoSelectRef.current && !addrLoading && !geoSuggestions.length) {
      ...
      pendingAutoSelectRef.current = false
      triggerAddrError(); setAddrInvalid(true)
    }
  ...
  const openAddressEditor = () => {
    ...
    setTimeout(()=> {
      if (addrInputRef.current) {
        addrInputRef.current.focus(); addrInputRef.current.select()
      }
    }, 0)
  }
  ...
  useEffect(()=> {
    ...
    const handler = (e: MouseEvent) => {
      ...
      if (addrEditorWrapRef.current && addrEditorWrapRef.current.contains(target)) return
      if (mapWrapperRef.current && mapWrapperRef.current.contains(target)) return // map tıklaması hariç (zaten başka logic var)
    ...
    <div className="app-shell">
      {showCookieModal && (
        <div className="cookie-modal-overlay">
          <div className="cookie-modal" role="dialog" aria-modal="true" aria-labelledby="cookie-required-title">
            ...
            <button className="cm-accept" onClick={handleForceAcceptCookies}>{t('cookie.required.accept')}</button>
          </div>
        </div>
      )}
      {/** Geçerli koordinat: reverse geocode sonrası addrInvalid=false ise accepted */}
      {(() => { const hasValidPoint = !!point && !addrInvalid; return (
      <aside className={`app-sidebar${!hasValidPoint ? ' empty' : ''}`}>
        ...
        <div className="sidebar-section head">
          <h2 className="logo-inline">
            <div ><Link title={t('app.home.back')} to="/"><TiArrowLeftThick style={{ marginRight: ".5rem" }} />PreWeather<span><PiSparkleFill /></span></Link></div>
          </h2>
        </div>
        {hasValidPoint && (
          <>
              ...
              <div className='sidebar-select-side'>
                <div className='sidebar-section main'>
                  <div className="sidebar-section location-block">
                    ...
                    <div className="kv">
                      <div><span>{t('app.lat')}</span><strong>{point.lat}</strong></div>
                      <div><span>{t('app.lng')}</span><strong>{point.lng}</strong></div>
                    </div>
                  </div>
                  <div className="sidebar-section date-block">
                    ...
                    />
                    <small className="range-info">{todayISO} - {maxDateISO}</small>
                  </div>
                </div>
                <div className="sidebar-section action-block">
                  <button className="predict-btn small" disabled={!point || !date || checking} onClick={handleFetchPrediction}>
                    {checking ? <AiOutlineLoading className={checking ? 'spin' : ''} /> : <PiSparkleFill />}
                    {checking ? t('app.results.loading') : t('app.results.fetch')}
                  </button>
                </div>
              </div>
            )}
            {result && (
              <div className="results">
                ...
                <div className="unit-toggle-row">
                  <span className="ut-label" title={t('unit.toggleLabel')}>{t('unit.toggleLabel')}:</span>
                  <div className="ut-switch" role="radiogroup" aria-label={t('unit.toggleLabel')}>
                    <button type="button" className={tempUnit==='C'?'on':''} aria-pressed={tempUnit==='C'} onClick={()=> setTempUnit('C')}>{t('unit.celsius')}</button>
                    <button type="button" className={tempUnit==='F'?'on':''} aria-pressed={tempUnit==='F'} onClick={()=> setTempUnit('F')}>{t('unit.fahrenheit')}</button>
                  </div>
                  <div className="ut-switch" role="radiogroup" aria-label="Wind Unit" style={{marginLeft:'.4rem'}}>
                    <button type="button" className={windUnit==='MS'?'on':''} aria-pressed={windUnit==='MS'} onClick={()=> setWindUnit('MS')}>m/s</button>
                    <button type="button" className={windUnit==='KMH'?'on':''} aria-pressed={windUnit==='KMH'} onClick={()=> setWindUnit('KMH')}>km/h</button>
                  </div>
                </div>
                <div className="readable-summary">
                  <div className="readable-head">
                    ...
                    {typeof result?.prediction_data?.percentage === 'number' && (
                      <em className="rel-pill" title={t('app.results.reliability')}>
                        {t('app.results.reliability')}: {result.prediction_data.percentage.toFixed(1)}%
                      </em>
                    )}
                  </div>
                  {(() => {
                    const pdata = result.prediction_data || {}
                    const firstDataKey = (obj:any) => Object.keys(obj||{}).find(k=> k !== 'percentage') || ''
                    const items: {icon: React.ReactNode; label: string; value: string | null; descKey?: string; rel?: number}[] = []
                    const metricIcon = (m:string): React.ReactNode => {
                      switch(m){
                        case 'T2M': case 'T2M_MAX': case 'T2M_MIN': return <FaTemperatureHalf />
                        case 'WS2M': return <FaWind />
                        case 'PRECTOTCORR': return <FaCloudRain />
                        case 'SNODP': return <FaSnowflake />
                        case 'RH2M': return <FaWater />
                        case 'ALLSKY_KT': return <PiSunHorizonBold />
                        default: return <PiSparkleFill />
                      }
                    }
                    const formatVal = (m:string, raw:number|undefined|null): string | null => {
                      if(typeof raw !== 'number' || isNaN(raw)) return null
                      if(['T2M','T2M_MAX','T2M_MIN'].includes(m)) return formatTemp(raw)
                      if(m==='WS2M') return (windUnit==='KMH'? (raw*3.6).toFixed(1)+' km/h' : raw.toFixed(1)+' m/s')
                      if(m==='PRECTOTCORR') return (raw < 0 ? 0 : raw).toFixed(1)+' mm'
                      if(m==='SNODP') return raw.toFixed(2)+' cm'
                      ...
                      if(['AOD_55_ADJ'].includes(m)) return raw.toFixed(3)
                      if(['CDD18_3','HDD18_3'].includes(m)) return raw.toFixed(2)
                      if(['FROST_DAYS'].includes(m)) return raw.toFixed(0)
                      if(['PRECSNOLAND'].includes(m)) return raw.toFixed(2)
                      return raw.toString()
                    }
                    metricsList.forEach(m=> {
                      ...
                      const raw = (obj && dk && typeof obj[dk] === 'number') ? obj[dk] : null
                      ...
                      items.push({
                        ...
                        rel: typeof obj.percentage === 'number'? obj.percentage : undefined
                      })
                    })
                    if (!items.length) return <div className="readable-empty">–</div>
                    return (
                      <ul className="readable-list">
                        {items.map((it,i)=>(
                          <li key={i} title={(it.descKey && t(it.descKey) !== it.descKey? t(it.descKey)+' • ' : '') + it.label + (it.rel? ' • '+it.rel.toFixed(0)+'%': '')}>
                            <span className="ic">{it.icon}</span>
                            <span className="lab" title={it.label}>{it.label.replace(/\s*\(.*?\)\s*/g, '').trim()}</span>
                ...
                <div className="download-actions">
                  <button type="button" onClick={handleDownloadCSV} className="dl-btn csv"><FaDownload />{t('app.results.downloadCSV')}</button>
                  <button type="button" onClick={handleDownloadJSON} className="dl-btn json"><FaDownload />{t('app.results.downloadJSON')}</button>
                </div>
              </div>
            )}
      ...
      {(() => { const hasValidPoint = !!point && !addrInvalid; return hasValidPoint && (
        <>
          {result && (
            <div className="result-block">
              <div className="metrics-list">
                <div className="metrics-head">
                  ...
                  <button className="toggle-all" onClick={toggleAll}>{t('app.results.toggleAll')}</button>
                </div>
                <ul>
                  {metricsList.map(m => {
                    const descMap: Record<string,string> = {
                      'T2M':'metricDesc.T2M','T2M_MAX':'metricDesc.T2M_MAX','T2M_MIN':'metricDesc.T2M_MIN','WS2M':'metricDesc.WS2M','PRECTOTCORR':'metricDesc.PRECTOTCORR','SNODP':'metricDesc.SNODP','RH2M':'metricDesc.RH2M','ALLSKY_KT':'metricDesc.ALLSKY_KT'
                    }
                    const desc = descMap[m] ? t(descMap[m]) : undefined
                    return (
                      <li key={m} className={metricsEnabled[m] ? 'on' : ''} title={desc}>
                        <label>
                          <input type="checkbox" checked={!!metricsEnabled[m]} onChange={() => toggleMetric(m)} />
                          <span>{t(`metric.${m}`).replace(/\s*\(.*?\)\s*/g, '').trim()}</span>
                          <em>{result.prediction_data[m]?.percentage?.toFixed(2)}%</em>
                        </label>
                      </li>
                    )
                  })}
  ...
  <div className="app-map-wrapper" ref={mapWrapperRef}>
        {/* Address overlay bottom center (always visible even if no point selected) */}
        <div className="addr-overlay" onClick={()=> openAddressEditor()}>
          {!editingAddress && (
            <div className={`addr-display${addrErrorShake ? ' addr-error-shake' : ''}`} title={t('addr.editHint') + ' (Alt+S)'}>
              {addrLoading ? <span className="addr-spinner" /> : <FaLocationDot className="addr-location-icon" />}
              <span className="addr-text">{addrInvalid ? t('addr.invalidMessage') : (address || (addrLoading ? t('addr.searching') : (!point ? t('addr.enterPlaceholder') : '')))}</span>
            </div>
          )}
          {editingAddress && (
            <div ref={addrEditorWrapRef} className={`addr-editor${addrErrorShake ? ' addr-error-shake' : ''}`} onClick={e=> e.stopPropagation()}>
              <input
                ref={addrInputRef}
                className="addr-input"
                value={addrInvalid ? '' : addrQuery}
                onChange={e=> { setAddrQuery(e.target.value); if(addrInvalid) setAddrInvalid(false) }}
                placeholder={addrInvalid ? t('addr.invalidMessage') : (!point ? t('addr.enterPlaceholder') : t('addr.placeholder'))}
                onKeyDown={e=> {
                  if (e.key==='Escape') { setAddrQuery(address); closeAddressEditor(true) }
                  if (e.key==='Enter') {
              ...
              <div className="addr-suggestions" ref={suggListRef}>
                {addrLoading && (
                  <>
                    <div className="loading-sugg"><span className="addr-spinner" />{t('addr.searching')}</div>
                  </>
                )}
                {!addrLoading && geoSuggestions.map((s,idx) => (
                  <div key={s.display} className={`addr-sugg${idx===suggIndex?' active':''}`} onClick={()=> applySuggestion(s)}>
                    <div className="addr-sugg-line">{renderHighlighted(s.display)}</div>
                    <div className="addr-sugg-meta">{s.lat.toFixed(4)}, {s.lon.toFixed(4)}</div>
                  </div>
                ))}
          ...
          <div className="top-center-stats">
            {typeof result?.prediction_data?.percentage === 'number' && (
              <div className="overall-chip">
                <span className="label">{t('app.results.overallAccuracy')}</span>
            ...
            {typeof result?.prediction_data?.percentage === 'number' && (
              <div className="overall-chip error">
                ...
                <strong>{(100 - result.prediction_data.percentage).toFixed(2)}%</strong>
              </div>
            )}
            {error && (
              <div className="overall-chip warn"><strong>{error}</strong></div>
            )}
          </div>
        )}
        {result && (parsedAdvice || result.agricultural_advice) && (
          <div className="ai-insights-overlay">
            <div className="ai-insights-head">{t('app.results.aiInsights')}</div>
            <div className="ai-insights-body">
              {parsedAdvice ? (
                orderedAdviceEntries.map(entry => {
                  ...
                  return (
                    <div className="ai-insight" key={entry.key}>
                      <div className="ai-insight-key">{label !== labelKey ? label : entry.key}</div>
                      <div className="ai-insight-val">
                        {isArray && (
                          <ul>
                            {v.map((item: any,i:number)=><li key={i}>{String(item)}</li>)}
                          </ul>
                        )}
                        {!isArray && isObject && (
                          <ul className="kv-list">
                            {Object.entries(v).map(([k,val]) => <li key={k}><strong>{k}</strong><span>{String(val)}</span></li>)}
                          </ul>
                        )}
                        {!isArray && !isObject && (
                          <p>{String(v)}</p>
              ...
              ) : processedAdviceLines.length ? (
                processedAdviceLines.map((seg,i) => (
                  <div className="ai-insight" key={i}>
                    {seg.labelKey && <div className="ai-insight-key">{t(seg.labelKey)}</div>}
                    <div className="ai-insight-val">
                ...
                <div className="ai-insight"><div className="ai-insight-val"><p>{result.agricultural_advice}</p></div></div>
              )}
            </div>
          </div>
        )}
